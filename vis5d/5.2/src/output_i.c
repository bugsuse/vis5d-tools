...
int make_output_ctx( struct grid_db *db, v5dstruct *v5d, char *filename, char *ctxname,
                       int maxnl, int average, int compressmode, int mbs, int mf, int mc )
{
   int time, var;
   int xlate_time[MAXTIMES], xlate_var[MAXVARS];
   int nl[IMAXVARS], lowlev[IMAXVARS];
   ...
   if (mf == 1 && mc == 1){
      ...
      yes = vis5d_load_v5dfile( 0, mbs, filename, ctxname);
...
/* TODO LATER!!!!! 
      filename[0] = 0;
      yo = vis5d_load_v5dfile(0,  v5d, mbs, filename, ctxname); */
      if ( yo <= 0 ){
         printf("not enough memory please give a file name to write to \n");
/*

         strcpy( message, " not enough memory, please give a file name to write to. ");
         show_error_widget( 0, message);
*/         return 0;
      }
   

      /*
       * Write grid data to a ctx
       */
      for (time=0;time<v5d->NumTimes;time++) {
         for (var=0;var<v5d->NumVars;var++) {
            int table_time, table_var;
            ...
            data = get_combined_resampled_data( db, table_time, table_var,
                                    ...
                                    average );
            if (data) {
               /*vis5d_insert_grid_into_ctx( 1, time, var, data); */
               FREE( data, 9 );
            }
            else {
/***************** TO DO STUFF ***********************************/
...
/*   strcpy(message, "you must give a file name and/or a context name please");
   show_error_widget( 0, message);
*/ printf("it dosn't work\n");
