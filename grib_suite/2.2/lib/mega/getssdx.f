...
c Meteorologico Regionale (SMR), Viale Silvani 6, 40122 Bologna, Italia
	...
	SUBROUTINE GETSSDX(PRIMO,CONTRO,ISTAZL,DATAL,ORAL,ISTAZU,
	1		DATAU,ORAU,TIPO,BUFFER,NREP,ERRORE)
COMSTART GETSSDX
C	SUBROUTINE GETSSDX(PRIMO,CONTRO,ISTAZL,DATAL,ORAL,ISTAZU,
C	1		DATAU,ORAU,TIPO,BUFFER,NREP,ERRORE)
C
C	routine obsoleta       vedi N_GETSSDX
COMEND
...
C	TYPE*,TIPO(J),CONTRO(J),PRIMO(J)
	...
	IOLT=IOLTV(J)
c	*************   per stazione data  **************
	CALL GETSSD(PRIMO(J),CONTRO(J),ISTAZL,DATAL,ORAL,ISTAZU,
