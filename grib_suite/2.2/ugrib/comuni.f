...
c	print *,'write codice errore',ier
	...
	subroutine write_grib (kunit_wg,grib_wg,idimg,ier)

        include "comuni.h"

	dimension grib_wg(idimg),isec0(2)
...
c        call set_def_val ()
	...
	CALL GRIBEX (ISEC0,ISEC1,ISEC2,ZSEC2,ISEC3,ZSEC3,ISEC4,
     $       field,klenp,grib_wg,idimg,kWORD,'L',IERR)
c	print*,'errore gribex=',ierr,' lunghezza',isec0(1)
	...
	call pbwrite(kunit_wg,grib_wg,isec0(1),kret)
...
c sembrerebbe che:
c il primo parametro (indice 2) dve essere un grib
c il secondo parametro (indice 1) deve essere o grib o costante
c i seguenti possono essere solo costanti
...
33	continue
	...
	do i=4,np
	   read (par(i),*,err=300)rval(i-3)
	end do

	ier=14
	ig=2          ! per default scrive le sezioni relative al primo grib
	...
	if      (par(2).eq.'media')then
     ...
     $          ,idim2,rmd,ncampi(ngb2))
		...
		isec1(17,ig)=max(isec1(17,ig),isec1(16,ig))
	...
	1	,idim2,rmd,ncampi(ngb2))
	else if (par(2).eq.'rh')then
		call rh(values(1,ngb1),values(1,ngb2)
	1	,idim2,rmd,ncampi(ngb2))
	...
	CALL GRIBEX (isec0(1,ig),isec1(1,ig),isec2(1,ig),zsec2(1,ig)
     $       ,isec3(1,ig),zsec3(1,ig),isec4(1,ig),
     $       values(1,ngb2),idim2,grib(1,ngb2),idimg,kword,'C',ker)

c       attenzione la gribex rovina la matrice dati in ingresso
...
c	print*,'moltiplica',values2(1),values1(1)
	...
	subroutine thetaep(values1,values2,idim,rmd,ncampi)
	dimension values1(idim),values2(idim)
	logical trace
	common /debug/trace
...
c	if (trace) print *,'P=', p
	do i=1,idim
	  if(values1(i).ne.rmd.and.values2(i).ne.rmd)then
             values2(i)=values2(i)*((1000./values1(i))**cost)*
     $       exp(Lc*(0.622*ESAT(values2(i))/values1(i))/(cp*values2(i)))
	  else
	    values2(i)=rmd
	  end if	
	end do
...
c la prendo dalla termolib.f perche' non so dove altro pescarla!
	FUNCTION ESAT(T)
	DATA ABZ/273.16/
         TC=T-ABZ
...
c	if (trace) print *,'P=', p
	do i=1,idim
	  if(values1(i).ne.rmd.and.values2(i).ne.rmd.
     $          and.values1(i).lt.1150)then
	     values2(i)=values2(i)*((1000./values1(i))**cost)
	  else
	     values2(i)=rmd
	  end if	
	...
	subroutine rh(values1,values2,idim,rmd,ncampi)
	dimension values1(idim),values2(idim)
	logical trace
	common /debug/trace
...
c	ncampi=ncampi+1
	...
	do i=1,idim
	  if(values1(i).ne.rmd.and.values2(i).ne.rmd)then
 	    values2(i)=100.*(ESAT(values2(i))/ESAT(values1(i)))
 	  else
	    values2(i)=rmd
	  end if	
	end do
	...
	subroutine filtro(values1,values2,idim,nx,ny,rmd,ija,ncampi_fi)
	dimension values1(idim),values2(idim)
...
c	print *,'printdati codice errore',ier
	return
      ...
      subroutine testkeys (grib_te,maxlen_te,ier)
...
c      call set_def_val ()
      ...
      CALL SETPAR (KBIT,KNEG,KPR) !get machine dependent parameters
      idimg=maxlen_te/(kbit/8)     !dimensione in word
      
      ier=14
      
      call getinfo (-1,grib_te,idimg
     +     ,keytmp(1),keytmp(4),keytmp(6)
     +     ,keytmp(10),keytmp(13),
     +     ala1,ala2,alo1,alo2,nj,ni,dj,di,idrt
     +     ,alarot,alorot,rot,ija,kret)
