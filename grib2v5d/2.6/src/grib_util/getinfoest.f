...
c
c    $Date: 2008-07-30 12:49:06 +0200(mer, 30 lug 2008) $    $Revision: 103 $
c    $Id: getinfoest.f 103 2008-07-30 10:49:06Z patruno $
...
c    http://www.smr.arpa.emr.it
      ...
      subroutine getinfoest (iug,grib,idimg,data,ora,scad,level,var,est
     +	,ala1,ala2,alo1,alo2,nj,ni,dj,di,idrt
     +	,alarot,alorot,rot,ija,ier)

COMSTART getinfoest
C
C	subroutine getinfoest (iug,grib,idimg,data,ora,scad,level,var,est
...
cc
      ...
      integer data(3),ora(2),level(3),scad(4),var(3),est(3)

      data irecl/120/

      include 'getinfo.inc'

      if (isec1(24).eq.1)then
        est(1)=isec1(37)        ! estensione per la lettura dei 
        est(2)=isec1(39)
        est(3)=isec1(42)
      else
        est(1)=0
