...
c
c    $Date: 2002-10-08 19:59:42 +0200(mar, 08 ott 2002) $    $Revision: 53 $
c    $Id: getinfovep.f 53 2002-10-08 17:59:42Z patruno $
...
c    http://www.smr.arpa.emr.it
      ...
      subroutine getinfovep (iug,grib,idimg,data,ora,scad,level,var,
     +	ala1,ala2,alo1,alo2,nj,ni,dj,di,idrt
     +	,alarot,alorot,rot,ija,nvcpm,nvcp,vcp,ier)

COMSTART GETINFOVEP
C
C	subroutine getinfovep (iug,grib,idimg,data,ora,scad,level,var,
C	1	ala1,ala2,alo1,alo2,nj,ni,dj,di,idrt
C	1	,alarot,alorot,rot,ija,nvcpm,nvcp,vcp,ier)
C
c
...
c                               =9000 numero di vertical coordinate parameter
c                                     > di 300 oppure > di nvcpm
c				altri > vedi errori pbgrib/gribex
c       nvcp                    num. di vertical coordinate parameter in output
c       vcp(nvcp)               vertical coordinate parameter
COMEND
...
cc
      ...
      DIMENSION ISEC2(384)
      DIMENSION ZSEC2(310),vcp(nvcpm) ! per vertical coordinate paramete
      DIMENSION ISEC3(2)
      DIMENSION ZSEC3(2)
      DIMENSION ISEC4(42)
...
cc
      ...
      if (nvcp.gt.300.or.nvcp.gt.nvcpm) then

        print*,'errore num. vertical coord. parameter',
     +   ' troppo grande',nvcp
        ier=9000
        return
...
c       vertical coordinate parameter
