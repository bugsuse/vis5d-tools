 ...
 */
int rowcol_to_latlon( float row, float col, float *lat, float *lon,
...
#undef Longitude
      ...
      default:
         printf("Error in rowcol_to_latlon: bad projection: %d\n", proj );
         break;
   }
 ...
 */
int latlon_to_rowcol( float lat, float lon, float *row, float *col,
                      struct projection *proj )
{
   float lat0, lon0;
...
#define Rotation (proj->Args[6] * DEG2RAD)
         ...
         *col = (lon0-WestBound) * (proj->Nc-1) / (EastBound-WestBound);
         *row = (lat0-NorthBound) * (proj->Nr-1) / (SouthBound-NorthBound);
         if (   *row<0.0 || *row>(proj->Nr-1)
             || *col<0.0 || *col>(proj->Nc-1)) {
            return 0;
         }
...
#undef Rotation
      ...
      default:
         printf("Error in latlon_to_rowcol: bad projection: %d\n", proj );
         break;
   }
...
#undef ColInc
      ...
      case PROJ_EPA:   /* EPA projection */
         /* measure distance between a few grid points in center of grid */
         {
            ...
            rowcol_to_latlon( midrow, midcol, &lat0, &lon0, proj );
            rowcol_to_latlon( midrow+1.0, midcol+1.0, &lat1, &lon1, proj );
            dlat = lat1-lat0;
            dlon = lon1-lon0;
