...
c Meteorologico Regionale (SMR), Viale Silvani 6, 40122 Bologna, Italia
	...
	SUBROUTINE QUAMETARCONS_new(FLAG,IER)
COMSTART QUAMETARCONS_new
C	SUBROUTINE QUAMETARCONS_new(FLAG,IER)
...
c
cCC(4)		CC(1,2,3)	CC(4)=CB/CU	CC(1,2,3)=CB/CU
c
cCC(1,2,3)	CC(4)		CC(1,2,3)=CB/CU	CC(4)=CB/CU
COMEND
	COMMON /RRW/RRW
	COMMON /CODER/CODER
...
c	e gli do priorita`
	...
	IF(CC(1).EQ.'CA'.AND.CC(2).EQ.'VO')THEN
	  iiww=00
	ELSE
	  iiww=NTPRmet_to_syn(ww,ns(1))	!conversione tempo presente
	END IF

C	CALL NCARINT(WW,1,2,IERRO)
C	IF(IERRO.NE.0)THEN
C1111		iiww=32767
C		GOTO 2222
C	END IF
C
C	READ(WW,'(I2)',ERR=1111)iiww
C
C2222	CONTINUE
...
340	CONTINUE
	...
	DO I=1,3					
	   DO J=1,2
		IF (CC(I).EQ.CB(J).AND.CH_C_E(CC(4)))THEN
			II=I
			GOTO 35
		END IF
	   END DO	
